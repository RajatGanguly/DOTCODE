...
function Profile({props_contents}) {
  const [editText,  setEditText] = useState(false)
  console.log(JSON.parse(props_contents))
  const user = JSON.parse(props_contents).result[0];
  return (
    <div>
      <Head>
        ...
        <div class="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto lg:my-0">
          <div
            ...
            <div className="flex flex-row-reverse">
              <Link href={`/edit/${user.id}`}><a><h1 className="px-12 py-4 cursor-pointer" onMouseOver={()=>{setEditText(true)}} onMouseOut={()=>{setEditText(false)}}><FiEdit/></h1></a></Link>              
              {editText ? <h1 className="px-12 py-2 bg-blue-400 border-2 rounded-2xl cursor-pointer text-white font-bold">Edit Your Profile</h1> : ""}
            </div>
            <div class="p-16 md:p-12 text-center lg:text-left">
              <div
...
export async function getServerSideProps(context){
  const u_id = context.params.profileNo;
  console.log(context.req.headers.host)
  const user_promise = await fetch(`http://${context.req.headers.host}/api/getuser?u_id=${u_id}`)
  const props_contents = await user_promise.json()
